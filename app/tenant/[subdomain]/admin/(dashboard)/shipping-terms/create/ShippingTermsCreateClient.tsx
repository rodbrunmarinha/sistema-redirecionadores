...
import { createShippingTerm } from '../_actions/shippingTerms';
...
export function ShippingTermsCreateClient({ subdomain }: { subdomain: string }) {
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);

  const [title, setTitle] = useState('');
  const [content, setContent] = useState('<p>Ao criar uma solicitação de envio, você concorda com os seguintes termos:</p><p><br></p><h3>1. Responsabilidade pela Declaração</h3><p>O cliente é totalmente responsável pela veracidade das informações declaradas.</p><p><br></p><h3>2. Produtos Proibidos</h3><p>Não aceitamos envio de: armas, explosivos, drogas ilícitas, produtos perecíveis, etc.</p><p><br></p><h3>3. Taxas Alfandegárias</h3><p>O destinatário é responsável pelo pagamento de todas as taxas alfandegárias.</p>');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await createShippingTerm({
        title,
        content,
        is_active: isActive,
