...
import { getTenantSettings } from "../settings/_actions/settings";
import { ImportCalculatorClient } from "./ImportCalculatorClient";

export default async function ImportCalculatorSettingsPage(props: { params: Promise<{ subdomain: string }> }) {
      ...
      <div className="bg-zinc-900 border-b border-zinc-800 relative overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <nav className="flex items-center gap-2 text-sm mb-4" aria-label="Breadcrumb">
            ...
            <ChevronRight className="w-4 h-4 text-zinc-600" />
            <span className="text-zinc-100 font-medium">Calculadora de Importação</span>
          </nav>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Calculadora de Importação</h1>
              <p className="text-zinc-400 text-sm mt-1">
                Configure as taxas para a simulação de importação dos seus clientes.
              </p>
            </div>
          </div>
        </div>
