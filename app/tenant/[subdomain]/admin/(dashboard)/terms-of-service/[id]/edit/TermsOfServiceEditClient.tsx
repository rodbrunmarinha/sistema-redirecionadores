...
import { updateTermOfService } from '../../_actions/termsOfService';
...
export function TermsOfServiceEditClient({ subdomain, term }: { subdomain: string, term: any }) {
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);

  const [title, setTitle] = useState(term.title);
  const [version, setVersion] = useState(term.version || '1.0');
  ...
  const [requireOnSignup, setRequireOnSignup] = useState(term.require_on_signup !== false);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.trim() || !content.trim() || !version.trim()) {
      toast.error('Preencha o título, a versão e o conteúdo do termo.');
      return;
    }
    ...
    try {
      const res = await updateTermOfService(term.id, {
        title,
        version,
        content,
        is_active: isActive,
