...
import { createTermOfService } from '../_actions/termsOfService';
...
export function TermsOfServiceCreateClient({ subdomain }: { subdomain: string }) {
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);

  const [title, setTitle] = useState('');
  const [version, setVersion] = useState('1.0');
  const [content, setContent] = useState('<p>Ao criar uma conta, você concorda com os seguintes termos:</p><p><br></p><h3>1. Responsabilidade pela Declaração</h3><p>O cliente é totalmente responsável pela veracidade das informações declaradas.</p><p><br></p><h3>2. Produtos Proibidos</h3><p>Não aceitamos envio de: armas, explosivos, drogas ilícitas, produtos perecíveis, etc.</p><p><br></p><h3>3. Taxas Alfandegárias</h3><p>O destinatário é responsável pelo pagamento de todas as taxas alfandegárias.</p>');
  const [displayOrder, setDisplayOrder] = useState(0);
  const [isActive, setIsActive] = useState(true);
  const [requireOnSignup, setRequireOnSignup] = useState(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const toastId = toast.loading('Criando termo...');

    try {
      const res = await createTermOfService({
        title,
        version,
        content,
        is_active: isActive,
        require_on_signup: requireOnSignup,
        display_order: displayOrder
      }, subdomain);

      if (res.success) {
        toast.success('Termo criado com sucesso!', { id: toastId });
        router.push(`/admin/terms-of-service`);
      } else {
        toast.error(res.error || 'Erro ao criar termo.', { id: toastId });
      }
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="bg-zinc-900 rounded-2xl shadow-sm border border-zinc-800 overflow-hidden">
      <div className="p-4 sm:p-8 space-y-8">
        ...
        <div className="space-y-3">
          ...
          <div className="quill-dark">
            <ReactQuill 
              ...
              value={content}
              onChange={setContent}
              placeholder="Ao criar uma conta, você concorda com os seguintes termos..."
              modules={{
                toolbar: [
                  [{ 'header': [1, 2, 3, false] }],
        ...
        <div className="pt-4 border-t border-zinc-800 space-y-4">
          <div className="bg-zinc-900 border border-zinc-800 p-4 sm:p-5 rounded-2xl flex items-start justify-between gap-4">
            <div>
              <label className="font-bold text-white block mb-1">Exigir no Cadastro</label>
              <p className="text-zinc-400 text-sm">Se ativo, novos clientes precisarão aceitar este termo ao criar uma conta.</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer shrink-0 mt-1">
              <input type="checkbox" className="sr-only peer" checked={requireOnSignup} onChange={(e) => setRequireOnSignup(e.target.checked)} />
