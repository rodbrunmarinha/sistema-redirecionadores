...
import { createExtraService } from '../_actions/extraServices';
import toast from 'react-hot-toast';

export function ExtraServiceCreateClient({ subdomain }: { subdomain: string }) {
  ...
  const [chargeType, setChargeType] = useState<'fixed' | 'declared_percentage' | 'custom_declared_percentage'>('fixed');
  
  // Formatters
  const [priceDisplay, setPriceDisplay] = useState('0.00');
  const [percentageDisplay, setPercentageDisplay] = useState('0.00');
  const [weightDisplay, setWeightDisplay] = useState('0.000');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const toastId = toast.loading('Criando serviço...');
    ...
    const payload = {
      ...
      is_active: isActive,
      sort_order: 1 // Next order will just be appended for now or we can compute max
    };

    try {
      const res = await createExtraService(payload, subdomain);
      if (res.success) {
