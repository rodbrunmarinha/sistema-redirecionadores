...
import { DollarSign, ChevronDown, Moon, BellRing } from "lucide-react";

interface CustomerMainWrapperProps {
  children: ReactNode;
  firstName: string;
...
export function CustomerMainWrapper({
  ...
  dateFormatted,
  currency
}: CustomerMainWrapperProps) {
  const pathname = usePathname();
  const isStore = pathname?.includes("/app/store");
  ...
  return (
    <main className="flex-1 flex flex-col h-screen overflow-y-auto">
      ...
      {/* Page Content */}
      <div className={isStore ? "p-3 lg:p-4 min-h-full" : "p-8 lg:p-12"}>
        {children}
      </div>
